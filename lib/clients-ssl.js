...
//      так тоже не отправляется результат
        action.match('createClientKey') && deferredExecOpenSSL.resolve(stdout);
        action.match('createCSR') && deferredExecOpenSSL.resolve(stdout);
        action.match('createClientCRT') && deferredExecOpenSSL.resolve(stdout);
        action.match('createPKS12') && deferredExecOpenSSL.resolve(stdout);
...
var  createClientKey  = function() {
    ...
    var deferreds = ['.key', '.crt', '.p12' ].map(function(item) {
    ...
    var deferred_exit = _.Deferred();

    _.when.apply( deferreds, deferreds).done( function() {
        var params,
            isAny = _.any( arguments, function(arg) { return arg });
