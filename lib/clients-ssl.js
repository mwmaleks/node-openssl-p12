...
var spawnOpenSSL = function( params ) {
    ...
    var bothDone = function() {
        if (code) {
            _dfd.reject( new Error('Invalid openssl exit code: ' + code + '\n% openssl ' + params.join(' ') + '\n' + stderr), code );
        } else {
            _dfd.resolve( code, stdout, stderr );
        }
    };
...
var execOpenSSL = function ( params, action ) {
    ...
    spawnOpenSSL(params).done( function( code, stdout, stderr) {
        ...
        action.match('createClientKey') && _dfd.resolve(stdout);
        action.match('createCSR') && _dfd.resolve(stdout);
        action.match('createClientCRT') && _dfd.resolve(stdout);
        action.match('createPKS12') && _dfd.resolve(stdout);

        if ( action.match('getFingerprint') ) {

            var _fingerprint = stdout.match(/Fingerprint=([0-9a-fA-F:]+)$/m);

            console.log('_fingerprint = '.magenta, _fingerprint[1]);
...
//                почему-то оправляя тут результат в done я получаю undefined
                ...
                _dfd.reject( new Error('No fingerprint got') );
            }
        }
...
var  createClientKey  = function(opt) {
...
var createClientCRT = function(opt) {
...
var getFingerprint = function(opt) {
