...
function genSpkac(options) {
    ...
    spkac = 'SPKAC =' + opt.publicKey.replace('\n', '') + '\n' +
        ...
        ( opt.OU ? 'OU=' + opt.OU + '\n' : '' ) +
        ( opt.organizationName ? 'organizationName=' + opt.organizationName + '\n':'' ) +
        'countryName=' + opt.C + '\n' +
        'stateOrProvinceName=' + opt.ST + '\n' +
        'localityName=' + opt.L;

    fs.writeFile( path.join( dirPath, opt.clientFileName + '.spkac' ), spkac, function(err) {
